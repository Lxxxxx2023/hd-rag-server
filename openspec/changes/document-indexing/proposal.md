...
实现 RAG 平台的文档处理管道（写路径）。核心链路：文件上传 → 多格式解析（PDF/HTML/Markdown/DOCX/TXT）→ 统一 CanonicalDocument（ContentTree）→ 清洗 → 按 Service 配置分块 → 向量化 → 写入索引（pgvector + Elasticsearch）。这是 RAG 平台的数据基础，所有检索能力依赖此模块产出的索引数据。
...
→ 解析为 CanonicalDocument (ContentTree) → 清洗噪音节点
→ AutoChunkStrategy 选择分块策略 → 批量向量化 (Embedding API)
...
当前各业务线独立建设文档处理能力，解析器、分块策略、向量化逻辑重复实现。本模块将文档到索引的全链路标准化，产出统一的 CanonicalDocument 中间格式和分块策略体系，为上层检索提供高质量的数据基础。
