...
定义 RAG 中台的可观测性体系，包括链路追踪、指标监控、告警、成本统计、质量评估和反馈回路。
...
- **REQ-OBS-003**: 并行执行的节点以并行 span 呈现
...
- **REQ-OBS-011**: Service 错误率 > 5%，级别 P1，通知 Service owner
- **REQ-OBS-012**: P99 > 10s，级别 P2，通知平台运维
- **REQ-OBS-013**: LLM API 可用性 < 99%，级别 P1，自动切换 fallback
- **REQ-OBS-014**: Kafka lag > 1000，级别 P2，通知 + 自动扩展消费者
- **REQ-OBS-015**: Service 日成本异常飙升，级别 P2

### 成本追踪

- **REQ-OBS-016**: 按调用链路分摊 LLM / Embedding / Rerank 成本到每个 Service
- **REQ-OBS-017**: 成本统计维度：Service × Model × 时间范围
- **REQ-OBS-018**: 本地部署模型按 GPU 资源占用比例折算成本
- **REQ-OBS-019**: 提供 Service 日/周/月成本报表

### 离线评估

- **REQ-OBS-020**: 评测数据集管理（CRUD），每条含：question, expected_answer, relevant_chunk_ids, difficulty, category
...
- **REQ-OBS-033**: 根因分类关联到可执行改进项（补充文档 / 调整分块 / 优化检索配置）

### 查询调试

- **REQ-OBS-034**: 提供 Trace 查询 API，按 trace_id / Service / 时间范围检索
- **REQ-OBS-035**: 单 Trace 展示完整执行链路 + 每个节点详情（输入/输出/耗时/状态）
